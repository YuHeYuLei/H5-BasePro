...
/*!
 * =====================================================
 * Mui v3.7.2 (http://dev.dcloud.net.cn/mui) TEDA定制版
 * =====================================================
 */
/**
 * MUI核心JS
...
(function($, window) {
  ...
  $.plusReady(function() {
    if ($.options.keyEventBind.backbutton) {
      console.log('keyEventBind.backbutton:::::::__back')
  ...
  $.addInit({
    ...
    handle: function() {
      $.plusReady(function() {
        //如果不为true，则移除默认监听
        console.log('keyEventBind.backbutton:::::::removeEventListener ::::::__back')
        if (!$.options.keyEventBind.backbutton) {
...
(function($, window, document) {
  ...
  var createInput = function(placeholder, ispass) {
    if (ispass) {
      return '<div class="' + CLASS_POPUP_INPUT + '"><input type="password" autofocus placeholder="' + (placeholder || '') + '"/></div>';
    } else {
  ...
  var createPrompt = function(message, placeholder, title, btnArray, callback, type, ispass) {
    if (typeof message === 'undefined') {
      return;
    } else {
    ...
    if (!$.os.plus || type === 'div') {
      return createPopup(createInner(message, title || '提示', createInput(placeholder, ispass)) + createButtons(btnArray || ['取消', '确认']), callback);
    }
